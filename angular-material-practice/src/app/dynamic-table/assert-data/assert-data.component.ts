...
import { COLUMNS } from '../data-column-model';
...
@Component({
  selector: 'app-assert-data',
  templateUrl: './assert-data.component.html',
  styleUrls: ['./assert-data.component.css']
})
export class AssertDataComponent implements OnInit {
  url = './asserts/data/assert-data.json';
  dataObj = DATA;
  columns = COLUMNS;
  activeColumns = ['email', 'fullName', 'languageId', 'firstName'];
  constructor(private http: HttpClient) {
    // this.http.get('./assets/data/assert-data.json',{responseType: 'text'as 'json'}).subscribe((res: []) => {
    //   console.log(res);
    //  this.dataObj = res;
    // })
   }
